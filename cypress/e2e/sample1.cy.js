...
describe('campare the values',()=>{
   ...
   cy.get('p[class="product-price"]').first().then((txt)=>{
   
    var price1=txt.text();
    cy.log(price1)

  
   cy.get('p[class="product-price"]').eq(3).then((txt1)=>{
    ...
    cy.get('div[class="cart-info"]>table>tbody>tr:nth-child(2)>td:nth-child(3)').then((totelprice)=>{
